...
use crate::{
    assertions::{account_empty, check_self_pda, find_self_pda},
    error::ChickenError,
    state::{Pool, PoolState},
    ChangeAdminAccounts, ClaimFeesAccounts, Context, InitializeAdminAccounts,
};
...
pub fn claim_fees(ctx: Context<ClaimFeesAccounts>) -> ProgramResult {
    ...
    let pool_collateral_token_account =
        TokenAccount::from_account_info(ctx.accounts.pool_collateral_token_account)?;
    let admin_token_account = TokenAccount::from_account_info(ctx.accounts.admin_token_account)?;
...
pub fn initialize_admin(ctx: Context<InitializeAdminAccounts>) -> ProgramResult {
    ...
    let bump = if account_empty(ctx.accounts.admin_record) {
        ...
        unsafe { ctx.accounts.admin_record.borrow_mut_data_unchecked()[32] }
    };
    unsafe {
        sol_memcpy(
            &mut ctx.accounts.admin_record.borrow_mut_data_unchecked(),
            [ctx.accounts.admin.key().as_ref(), &[bump]]
                .concat()
                .as_ref(),
            33,
        );
...
pub fn change_admin(ctx: Context<ChangeAdminAccounts>) -> ProgramResult {
    ...
    check_self_pda(
        &[b"admin".as_ref(), &get_admin_record[32..]],
        ctx.accounts.admin_record.key(),
        ChickenError::InvalidAdminRecordAddress,
    )?;
    unsafe { sol_memcpy(get_admin_record, ctx.accounts.new_admin.key().as_ref(), 32) };
    Ok(())
}
